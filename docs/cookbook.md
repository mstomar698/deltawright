...
| **Checksum fidelity** | A green checksum is **regression-only**. | It proves the normalized structure/semantics is unchanged, *not* that the delta is correct or models a real app. It distinguishes *which* allowlisted **state** attribute changed on an `attrChanged` node (aria-pressed/expanded/…, disabled, class; Wave-1 #3) — by attribute **name**, not its old→new **value** (values aren't captured), and non-state/volatile attrs (`style`, generated tokens) are dropped. |
