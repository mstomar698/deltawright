...
export const CORPUS: CorpusCase[] = [
  ...
  {
    id: 'background-churn-recur-pos',
    code: 'background-churn',
    confidence: 'suspected',
    confuser: false,
    kind: 'delta',
    delta: nodeDelta({}, { recurringInsert: 8, hitMaxWait: true }),
    verdict: 'NOT-actionable',
    note: 'A container recurred 8x after the action AND kept settle from quiescing — post-action background churn (#7).',
  },
  {
    id: 'background-churn-recur-confuser',
    code: 'unknown',
    confidence: 'unknown',
    confuser: true,
    confusesWith: 'background-churn',
    kind: 'delta',
    delta: nodeDelta({}, { recurringInsert: 8, hitMaxWait: false }),
    verdict: 'NOT-actionable',
    note: 'Same recurrence count but settle went quiet (a bounded list reveal) — must NOT be background-churn.',
  },
  {
  ...
  // ===== capture-integrity ===============================================================
  {
    ...
    confuser: false,
    kind: 'live',
    fixture: 'fixtures/csp-blocked.html',
    action: click('#go'),
    note: "A page whose strict CSP (script-src 'none') blocks addScriptTag, so the observer cannot be injected. actAndObserve degrades: it still performs the click, then returns an empty delta carrying stats.injectionBlocked (#71 fix #4b), which diagnose maps to injection-blocked/confirmed. Runs live — the injection failure is real, not authored. (No window.__truth oracle: the CSP that defines this case also blocks the fixture's own scripts.)",
  },
  {
  ...
  },
  {
    ...
    confuser: false,
    kind: 'live',
    fixture: 'fixtures/cross-boundary.html',
    action: click('#go'),
    options: { frames: true },
    note: "A child frame the observer can't inject into (its srcdoc carries a strict CSP) is SKIPPED during frames:true traversal; the real armChildFrames skip count lands in stats.crossBoundarySkipped (#71 fix #4a), which diagnose maps to cross-boundary-partial/suspected. Runs live — the skip is real, not authored. A same-origin CSP-uninjectable stand-in for the cross-origin case (a true cross-origin child needs two origins → #25); closed shadow roots are structurally uncountable, so skipped frames are the honest signal.",
  },
  {
